...
const detectOutputMode = (text) => {
  ...
  let outputMode = "both";
  if (hasSql && hasNosql) outputMode = "both";
  else if (hasSql) outputMode = "sql";
  else if (hasNosql) outputMode = "nosql";
  ...
  return {
    outputMode,
    requestedEngine: {
      sql: sqlEngine ? SQL_ENGINES[sqlEngine] : null,
      nosql: nosqlEngine ? NOSQL_ENGINES[nosqlEngine] : null,
    },
  };
};
...
const singularizeEntity = (value) => {
  const normalized = normalise(value).replace(/\s+/g, "_");
...
const extractExtraEntities = (text, domainKey) => {
  ...
  const templateEntityNames = new Set(
    [
      ...
      ...(DOMAIN_TEMPLATES[domainKey]?.nosql?.collections || []).map(
        (c) => c.name,
      ),
    ].map(normalise),
  );
  const templateEntityFamilies = new Set(
    Array.from(templateEntityNames).map((entity) => singularizeEntity(entity)),
  );

  const matched = ENTITY_HINTS.filter((hint) => {
    const normHint = normalise(hint);
    const normalizedEntity = normHint.replace(/\s+/g, "_");
    const singularEntity = singularizeEntity(normalizedEntity);
    return (
      norm.includes(normHint) &&
      !templateEntityNames.has(normHint) &&
      !templateEntityNames.has(normalizedEntity) &&
      !templateEntityNames.has(singularEntity) &&
      !templateEntityFamilies.has(singularEntity)
    );
  })
  ...
  for (const entity of matched) {
    const family = singularizeEntity(entity);
