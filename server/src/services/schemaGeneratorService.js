...
const inferOutputMode = ({ prompt, database }) => {
  ...
  if (/\b(mongodb|mongo|document db|document database|nosql|dynamodb|firestore|cassandra)\b/i.test(normalizedPrompt)) {
    return {
      outputMode: 'nosql',
  ...
  if (/\b(postgresql|postgres|mysql|sqlite|sql server|sqlserver|relational|rdbms|sql)\b/i.test(normalizedPrompt)) {
    return {
      outputMode: 'sql',
...
const callSchemaModel = async ({ prompt, payload }) => {
  const llmResult = await generateText({
    ...
    system: 'You generate valid JSON for relational database schema design tasks.',
    prompt,
  });
...
const generateSchema = async (payload = {}) => {
  ...
  const generationPrompt = buildSchemaGenerationPrompt({
  ...
  try {
    schema = parseJsonObject(llmResult.response);
  ...
  if (parseError) {
    ...
    const stopReasonMessage =
      stopReason === 'max_tokens'
        ? ` Anthropic stopped because it hit max_tokens (${request.maxTokens}). Increase maxTokens or ask for a smaller schema.`
        : '';
