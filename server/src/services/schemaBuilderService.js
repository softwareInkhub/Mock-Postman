      ...
      return {
        ...
        tables: arch.tables.map((table) => ({
          ...
          columns: table.columns.map((col) => ({
            ...col,
            description: col.description || `${col.name} for the ${table.name} table.`,
          })),
        })),
      };
    }
    return {
      ...
      collections: arch.collections.map((col) => ({
        ...
        document_shape: col.document_shape.map((field) => ({
          ...field,
          description: field.description || `${field.name} for the ${col.name} collection.`,
        })),
      })),
    };
  });
...
const domainConfidenceScore = (tokens) => {
  ...
  for (const [domainKey, template] of Object.entries(DOMAIN_TEMPLATES)) {
    ...
    const score = template.keywords.reduce((count, kw) => {
      return count + (normalizedInput.some((input) => input.includes(kw)) ? 1 : 0);
    }, 0);
...
const buildFromDomainTemplate = (template, extraEntities, fieldHints, outputMode, requestedEngine) => {
  const architectures = [];
  ...
  if (outputMode === 'sql' || outputMode === 'both') {
    ...
    architectures.push({
      database_type: 'sql',
      database_engine: requestedEngine || template.sql.engine,
  ...
  if (outputMode === 'nosql' || outputMode === 'both') {
    ...
    architectures.push({
      database_type: 'nosql',
      database_engine: requestedEngine || template.nosql.engine,
...
const buildFromFeatureComposition = (prompt, featureKeys, extraEntities, fieldHints, outputMode, requestedEngine) => {
  ...
  if (featureKeys.length === 0) {
    ...
    for (const concept of concepts.slice(0, 5)) {
      ...
      if (!nosqlSeen.has(tableName)) {
        nosqlCollections.push(buildGenericNoSqlCollection(tableName, fieldHints));
        nosqlSeen.add(tableName);
      }
    }
  } else {
    ({ tables: sqlTables, seen: sqlSeen } = mergeSqlTables(featureKeys));
    ({ collections: nosqlCollections, seen: nosqlSeen } = mergeNoSqlCollections(featureKeys));
    ...
    if (!nosqlSeen.has('users')) {
      nosqlCollections.unshift(deepClone(FEATURE_MODULES.auth.nosql.collections[0]));
      nosqlSeen.add('users');
    }
  }
  ...
  if (outputMode === 'sql' || outputMode === 'both') {
    architectures.push({
      database_type: 'sql',
      database_engine: requestedEngine || 'postgresql',
      use_case: 'Best when strong relationships and transactional consistency matter.',
      tables: sqlTables,
    });
  }
...
const buildFromPromptComponents = ({ components, extraEntities, fieldHints, outputMode, requestedEngine }) => {
  const sqlTables = [];
  ...
  for (const component of components) {
    if (component.type === 'domain') {
      ...
      appendUniqueNoSqlCollections(noSqlCollections, noSqlSeen, template.nosql?.collections || []);
      continue;
    }
    ...
    appendUniqueNoSqlCollections(noSqlCollections, noSqlSeen, module.nosql?.collections || []);
  }
 ...
 */
const buildSchema = ({ prompt, sourceData = null, outputMode: outputModeOverride, requestedEngine: engineOverride }) => {
  const parsed = parsePrompt(prompt, sourceData);

  const outputMode = outputModeOverride || parsed.outputMode;
  const requestedEngine = engineOverride || parsed.requestedEngine;
  const allExtra = [...new Set([...parsed.extraEntities, ...parsed.sourceEntities])];
  ...
  if (promptComponents.length >= 2) {
    domainLabel = promptComponents.map((component) => component.key.replace(/_/g, ' ')).join(' + ');
    ...
    if (component.type === 'domain') {
      ...
      architectures = buildFromDomainTemplate(template, allExtra, parsed.fieldHints, outputMode, requestedEngine);
    } else {
    ...
    domainLabel = featuresUsed.length > 0
      ? featuresUsed.map((k) => FEATURE_MODULES[k] ? k.replace(/_/g, ' ') : k).join(' + ')
      : 'custom';
    compositionMode = featuresUsed.length > 0 ? 'feature_composition' : 'concept_extraction';
    architectures = buildFromFeatureComposition(
      prompt, featuresUsed, allExtra, parsed.fieldHints, outputMode, requestedEngine
    );
  }
  ...
  return {
    ...
    meta: {
      ...
      domainLabel,
      compositionMode,
      featuresUsed: promptComponents.length > 0
